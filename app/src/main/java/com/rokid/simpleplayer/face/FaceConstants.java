package com.rokid.simpleplayer.face;

public class FaceConstants {

    public static final String APP_ID = "5o2nsud9Tk4kj6eTamdSTbemCj99ZhYDkhwVfuTBMP9c";
    public static final String SDK_KEY = "8KdwfvQnkR5pL3kG6iTG7zf8fJkD7oN58GmjcBCcQvKe";

    /**
     * IR预览数据相对于RGB预览数据的横向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int HORIZONTAL_OFFSET = 0;
    /**
     * IR预览数据相对于RGB预览数据的纵向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int VERTICAL_OFFSET = 0;
}

